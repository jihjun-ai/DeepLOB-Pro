...
def ewma_vol(close: pd.Series, halflife: int = 60) -> pd.Series:
    ...
    # ✅ 修復：嚴格因果性的波動率初始化（只用過去數據）
    if vol.isna().any():
        # 先用前向填充（ffill）：每個 NaN 用之前的值填充
        vol = vol.fillna(method='ffill')

        # 開頭仍有 NaN（前 halflife 個點），用保守估計
        if vol.isna().any():
            # 使用全序列的中位數（保守估計，避免極端值）
            initial_vol = vol.median()
            if pd.isna(initial_vol):
    ...
    def build_split_v6(split_name):
        ...
        for sym, n_points, day_data_sorted in stock_list:
            ...
            for date, features, mids, bucket_mask in day_data_sorted:  # A.2: 加入 bucket_mask
                ...
                # 3. Triple-Barrier 標籤（修復版：對每個點獨立計算）
                tb_cfg = config['triple_barrier']
                # 強制開啟日界保護，避免跨日洩漏
                day_end_idx = len(close) - 1  # 修改：始終啟用

                # ✅ 修復：移除 TB 降採樣（徹底消除標籤平滑化洩漏）
                # 原問題：tb_stride 導致標籤前向填充，連續 N 個點有相同標籤
                # 修復方案：對每個時間點獨立計算 Triple-Barrier
                # 代價：計算時間增加（約 2-3 分鐘 → 20-30 分鐘）
                # 收益：洩漏測試預期改善 3-5%

                try:
                    # 對每個點計算 TB（無降採樣，無前向填充）
                    tb_df = tb_labels(
                        close=close,
                        vol=vol,
